...
# Define a global variable to store the server process
server_process = None
...
def AdminHome(root, frame1, frame3):
    ...
    # Admin Login
    runServer = Button(frame1, text="Run Server", width=15, command=start_server)

    stopServer = Button(frame1, text="Stop Server", width=15, command=stop_server)
    ...
    stopServer.grid(row=9, column=1, columnspan=2)
...
def start_server():
    global server_process
    server_path = '/Library/Frameworks/Python.framework/Versions/3.12/Resources/Python.app/Contents/MacOS/Python'
    script_path = '/Users/skmirajulislam/Documents/Online-Voting-System/Server.py'
    server_process = sbp.Popen([server_path, script_path])
